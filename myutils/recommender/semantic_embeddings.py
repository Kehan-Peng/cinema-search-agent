...
def semantic_search(query: str, top_n: int = 10, model_name: str = "word2vec") -> List[Dict]:
    """基于语义的智能搜索

    采用混合策略：
    1. 精确匹配：标题、类型、导演等字段的完全匹配
    2. 模糊匹配：基于关键词的部分匹配
    3. 语义匹配：基于字符级别的重叠度
    """
    ...
    for movie in movies:
        ...
        if movie_id not in embedding_map:
            continue
        ...
        if query_lower in title:
            ...
            if query_chars and title_chars:
                ...
                total_score += (overlap / len(query_chars)) * 2.0

        # 2. 简介匹配（权重高）
        summary = (movie.get("summary") or movie.get("description") or "").lower()
        if summary:
            # 完整查询词在简介中
            ...
            # 查询关键词在简介中的匹配度
            summary_tokens = set(_tokenize_text(summary))
            matched_tokens = sum(1 for token in query_tokens if token in summary_tokens)
            if query_tokens:
                total_score += (matched_tokens / len(query_tokens)) * 3.0

            # 字符级别重叠
            summary_chars = set(summary)
            if query_chars and summary_chars:
                overlap = len(query_chars & summary_chars)
                total_score += (overlap / len(query_chars)) * 1.5

        # 3. 类型匹配
        types = [(t or "").lower() for t in (movie.get("types_list") or movie.get("genres") or [])]
        for genre in types:
            if query_lower in genre or genre in query_lower:
                total_score += 2.0
            else:
                genre_chars = set(genre)
                if query_chars and genre_chars:
                    overlap = len(query_chars & genre_chars)
                    total_score += (overlap / len(query_chars)) * 0.5

        # 4. 导演匹配
        directors = [(d or "").lower() for d in (movie.get("directors_list") or [])]
        for director in directors:
            if query_lower in director or director in query_lower:
                total_score += 1.5
    ...
    return _scores_to_movies(scores, "语义搜索", top_n)
