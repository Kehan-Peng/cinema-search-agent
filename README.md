...
基于强化学习和语义搜索的电影推荐系统，采用 Flask + SQLite 架构，支持本地单机运行。

## 项目特色

### 🎯 核心功能

- **智能语义搜索**：支持场景、情节、主题等模糊描述查询（如"一个男人站在大雨里"→《肖申克的救赎》）
- **个性化推荐**：混合推荐算法（内容推荐 + 协同过滤）+ PPO 强化学习重排
- **语义增强推荐**：本地轻量 Word2Vec/GloVe 实现，支持语义召回和协同过滤语义化
- **工业级 Agent 系统**：统一调度中枢、标准化工具封装、任务自动规划、全链路可观测
- **数据可视化界面**：采用专业的数据可视化设计系统，清爽简洁的浅色主题
- **完整数据链路**：可扩容、可断点续爬、支持封面下载的本地数据采集系统
