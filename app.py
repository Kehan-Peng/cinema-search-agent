...
def index():
    ...
    if session.get("email"):
        user_behaviors = get_user_behavior(session.get("email"))
        user_stats = {
            "rated_count": len([b for b in user_behaviors if b.get("behavior_type") == 1]),
            "favorite_count": len([b for b in user_behaviors if b.get("behavior_type") == 2]),
            "watched_count": len([b for b in user_behaviors if b.get("behavior_type") == 3]),
        }
...
def search():
    ...
    search_mode = request.args.get("mode", "keyword")  # keyword 或 semantic

    if not query:
        movies = []
    elif search_mode == "semantic":
        top_n = request.args.get("top_n", default=10, type=int)
        movies = catalog_service.semantic_search_movies(query, top_n)
    else:
        movies = catalog_service.search_movies(query)

    return render_template("search.html", query=query, movies=movies, search_mode=search_mode)
